...
public class SubscriptionDaoImpl extends JdbcDaoSupport implements SubscriptionDao {
    ...
    public List<UserResp> getMySubscribers(long userId) {
        return jdbcTemplate.query("SELECT id, nickname FROM user WHERE id IN " +
                        "(SELECT sub_user_id FROM subscription WHERE pub_user_id = ?);",
    ...
    public List<UserResp> getMyPublishers(long userId) {
        return jdbcTemplate.query("SELECT id, nickname FROM user WHERE id IN " +
                "(SELECT pub_user_id FROM subscription WHERE sub_user_id = ?);",
                new UserRespRowMapper(), userId);
    }
    ...
    public void subscription(long subUserId, long pubUserId) {
        try {
            ...
            throw CommonException.builder().code(Code.ALREADY_SUBSCRIBED).userMessage("Вы уже подписаны на этого пользователя")
                    .httpStatus(HttpStatus.BAD_REQUEST).build();
        } catch (DataIntegrityViolationException ex) {
            log.error(ex.toString());
            throw CommonException.builder().code(Code.PUBLISHER_NOT_FOUND).userMessage("Не найден пользователь для подписки")
                    .httpStatus(HttpStatus.BAD_REQUEST).build();
        }
